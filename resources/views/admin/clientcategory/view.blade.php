              ...
              <div class="card shadow mb-4">
                  ...
                  <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="client_class_id" id="name" class="form-control" required readonly>
                                    <option selected disabled hidden>Select</option>
                                    @foreach ($clientclass as $row)
                                    <option {{ $data->client_class_id==$row->id ? 'selected' : '' }} value="{{ $row->id }}">
                                        {{ $row->name }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="d-flex justify-content-between">
                          ...
                          <a class="btn btn-info" href="{{ route('admin.client-category.index') }}" >Back</a>
                          <a href="{{ route('admin.client-category.edit',$data->id) }}" class="btn btn-success" ><i class="fas fa-edit"></i></a>
                        </div>
                  </div>
              </div>
          </div>
