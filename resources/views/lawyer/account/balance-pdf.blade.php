            ...
            <div class="row">
                ...
                <div class="col-4">
                    @if($chamber->ch_status==1)
                        {!! $chamber->ch_letter_write_up_one !!}
                    @elseif($chamber->ch_status==2)
                        {!! $chamber->ch_letter_write_up_two !!}}
                    @else
                        {{ __('Empty Data') }}
                    @endif
                </div>
                ...
                <div class="col-4">
                   <div class="letter_head_address">
                       @if($chamber->ch_status==1)
                            {!! $chamber->ch_letter_address_one !!}
                        @elseif($chamber->ch_status==2)
                            {!! $chamber->ch_letter_address_two !!}}
                        @else
                            {{ __('Empty Data') }}
                        @endif
                   </div>
              ...
              <div class="col-12 mb-2">
                  <div class="d-flex justify-content-between align-items-center">
                      ...
                      <div>
                          <p class="border-bottom mb-0">Balance Report</p>
                ...
                <div class="col-sm-12">
                  <table class="table table-bordered " width="100%" cellspacing="0"
                    ...
                    <tbody>
                        ...
                        @php
                        $total_received += $bill->ledgers()->sum('paid_received_amount');
                        $total_balance += ($bill->bill_amount - $bill->ledgers()->sum('paid_received_amount'));
                        @endphp
                      <tr class="odd">
                        <td>{{$loop->iteration}}</td>
                        <td>{{$bill->cases->caseTitleShort->name_short.' '.$bill->cases->case_infos_case_no.'/'.$bill->cases->case_infos_case_year}}</td>
                        <td>{{$bill->bill_date->format('d-m-Y')}}</td>
                        <td>{{'BL-000'.$bill->id}}</td>
                        <td style="text-align: end;padding-right: 10px;">{{$bill->bill_amount}}</td>
                        <td style="text-align: end;padding-right: 10px;">{{$bill->ledgers()->sum('paid_received_amount')}}</td>
                        <td style="text-align: end;padding-right: 10px;">{{$bill->bill_amount - $bill->ledgers()->sum('paid_received_amount')}}</td>
                        <td>
                            @php
                             $received = $bill->ledgers()->sum('paid_received_amount');
                            $due = $bill->bill_amount - $received;
                            @endphp
                            @if($bill->bill_amount > $received)
                                <span class="badge badge-danger font-weight-bold">Due<span/>
                            @else
                    ...
                    <tfooter>
                        <tr class="odd">
                            ...
                            <td style="text-align: end;padding-right: 10px;font-weight:bold">
                                {{round($total)}}
                            </td>
                            <td style="text-align: end;padding-right: 10px;font-weight:bold">
                                {{round($total_received)}}
                            </td>
                            <td style="text-align: end;padding-right: 10px;font-weight:bold;color:red">
                                <span id="result">{{round($total_balance)}}</span>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="8">
                                <span class="font-weight-bold border-bottom">Balance In Words</span> <span>: </span>Taka <span id="show" ></span> only.
                            </td>
                        </tr>
                    </tfooter>
                  </table>
